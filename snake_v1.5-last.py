...
background = pygame.image.load(os.path.join(background_image_path, "background of pure sand.png"))                              #declration (loading) of image variable [game background]
...
background_game_over = pygame.image.load(os.path.join(background_image_path, "desert floor blurred.png"))          #declration (loading) of image variable [game over background]
...
snake_head_image_path = os.path.join(current_dir, "assets", "images")
snake_head_image = pygame.image.load(os.path.join(snake_head_image_path, "desert_krait_head.png"))
snake_head_image = pygame.transform.scale(snake_head_image, (snake_block, snake_block))

# Load and set window icon
logo_image_path = os.path.join(current_dir, "assets", "images", "Sapphire.png")
...
snake_tail_image_path = os.path.join(current_dir, "assets", "images")
snake_tail_image = pygame.image.load(os.path.join(snake_tail_image_path, "desert_krait_tail.png"))
...
def start_screen():
    ...
    while intro:
        ...
        # Draw game logo at the top center of the screen
        ...
        # Centered text positions with new color
        ...
        # Add clear gap before speed controls
...
pygame.mixer.init()
pygame.mixer.music.load(os.path.join(bgm_path, "Heights at 4.mp3"))
...
while running:
    start_screen()
    # game_loop returns True to go back to menu, False to exit game
    if not game_loop():
        running = False
